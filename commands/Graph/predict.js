    ...
    async run(message, [count]) {
        if (count < 1 || count > 1000000) {
            let embed = getErrorEmbed('Prediction Failed',
                'Count must be between 0 and 1,000,000.'
            );
        ...
        let counts = message.guild.configs.counts.map(c => c.count);
        ...
        if (changes.length < 3) {
            let embed = getErrorEmbed('Prediction Failed',
                'There isn\'t not enough data to make a prediction yet.'
            );
            return message.channel.send({
                embed
            });
        ...
        if (avgGain <= 0) {
            let embed = getErrorEmbed('Prediction Failed',
                'This server isn\'t gaining members. Average per day: ' + avgGain
            );
            return message.channel.send({
                embed
            });
